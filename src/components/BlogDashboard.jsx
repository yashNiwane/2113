...
const CardContent = ({ children, className, ...props }) => (
  <div className={`p-4 flex flex-col h-full ${className}`} {...props}>
    {children}
  </div>
);
...
const BlogDashboard = ({ selectedCategory, searchTerm }) => {
  ...
  useEffect(() => {
    const importPosts = async () => {
      try {
        const context = require.context('/src/components/posts', false, /\.jsx$/);
        const loadedPosts = context.keys().map((key) => {
          ...
          >
            <div className={`flex flex-col ${index === 0 ? "md:flex-row" : ""}`}>
              ...
              <CardContent className={`${index === 0 ? "md:w-1/3" : "w-full"}`}>
                <CardTitle
                  className={`${index === 0 ? "text-2xl md:text-3xl" : "text-xl"} text-purple-700`}
                >
                  {post.title}
                </CardTitle>
                <p
